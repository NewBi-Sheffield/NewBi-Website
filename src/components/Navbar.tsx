        ...
        <div className="flex items-center gap-3">
          {!loading && isLoggedIn ? (
            <div className="relative" ref={ref}>
              <button
                onClick={() => setOpen((o) => !o)}
                className="w-9 h-9 rounded-full overflow-hidden border-2 border-white/20 hover:border-[#a5de57]/60 transition-colors focus:outline-none bg-[#1a3550] flex items-center justify-center"
              >
              ...
              {open && (
                <div className="absolute right-0 top-11 bg-[#0f2236] border border-white/10 rounded-xl shadow-xl py-1 w-44 z-50">
                  <Link
                    href="/account"
                    onClick={() => setOpen(false)}
                    className="block px-4 py-2.5 text-sm text-slate-200 hover:bg-white/5 transition-colors"
                  >
                    My account
                  </Link>
                  <button
